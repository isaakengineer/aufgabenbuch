...
// ÜBER IDENTITÄT
fn datenbankpfad_fuellen(app: AppHandle, pfad: String) { // name ändern "identitaet_fuellen"
...
fn datenbankpfad_leeren(app: AppHandle) { // name ändern "identitaet_pfad_leeren"
	let data = app.state::<Mutex<AppIdentitaet>>();
	let mut data = data.lock().unwrap();
	data.dateipfad = None;
}

fn pool_fuellen(app: AppHandle, pool: Pool<Sqlite>) { // name ändern "identitaet_pool_fuellen"
	let data = app.state::<Mutex<AppIdentitaet>>();
	let mut data = data.lock().unwrap();
	data.pool = Some(pool);
}
...
async fn has_migration_been_applied(pool: &Pool<Sqlite>, migration_name: &str) -> Result<bool, String> {
	let result = sqlx::query("SELECT COUNT(*) as count
...
async fn datenbank_aktivieren_oder_herstellen(app: AppHandle, path: String) -> Result<String, String> {
	...
	let is_empty = match sqlx::query("SELECT name FROM sqlite_master
			WHERE type='table'
...
#[tauri::command]
pub async fn file_erstellen(app: AppHandle) -> Result<AppIdentitaet, String> { // TODO: name ändern  "dialog_neue_datei_erstellen"
...
#[tauri::command]
pub async fn file_waehlen(app: AppHandle) -> Result<AppIdentitaet, String> { // TODO: name ändern "dialog_existirende_datei_oeffnen"
